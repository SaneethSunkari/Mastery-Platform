...
describe("adaptive runtime honesty", () => {
  it("labels PySpark structural validation without awarding runtime evidence", async () => {
    ...
    const result = await executeAnswer({
      id: "spark", technology: "pyspark", curriculumNodeId: "pyspark-selection-select", topic: "Selection", subtopic: "select", difficulty: 1, title: "Select", scenario: "orders", prompt: "Select id", schema: {}, sampleData: {}, expectedBehavior: [], hiddenTests: [], referenceSolution: "result_df = orders_df.select('id')", starterCode: "", rubric: [], skillDimensions: ["select"], fingerprint: { technology: "pyspark", topic: "Selection", subtopic: "select", pattern: "projection", scenario: "orders", difficulty: 1, skills: ["select"], schemaSignature: "orders(id)" },
