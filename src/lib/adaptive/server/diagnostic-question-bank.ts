...
function sqlQuestion(nodeId: string, difficulty: Difficulty, variant: number): GeneratedQuestion {
  ...
  const definitions: Record<string, { title: string; prompt: string; solution: string; setup?: string; expected: string[] }> = {
    ...
    "sql-sorting-and-pagination-order-by": { title: `Rank the largest ${domain.name}`, prompt: `Return the ${limit} rows with the largest amount. Break ties by record_id ascending.`, solution: `SELECT record_id, amount FROM ${table} ORDER BY amount DESC, record_id ASC LIMIT ${limit}`, expected: [`Returns exactly ${limit} rows`, "Orders amount descending with a deterministic tie-breaker"] },
